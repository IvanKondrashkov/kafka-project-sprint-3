...
--data @infra/config/jdbc-source-connector.json \
http://localhost:8083/connectors/postgres-source/config
```

#### Проведите тестирование вставки данных
```sql
...
curl -X POST \
-H "Content-Type: application/json" \
--data @infra/config/prometheus-sink-connector.json \
...
```json
{
  ...
  "config": {
    "connector.class": "ru.practicum.service.PrometheusSinkConnector",
    "tasks.max": "1",
...
curl -X PUT \
-H "Content-Type: application/json" \
--data @infra/config/debezium-source-pg-connector.json \
http://localhost:8083/connectors/postgres-source/config
```
